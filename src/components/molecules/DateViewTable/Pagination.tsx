...
// ─────────────────────────────────────────────────────────────────────────────
// Inline SVG icons
// Inlined so we can flip them cleanly in RTL without the `rotate-180` hack
// that was previously applied to react-icons at the component call-site.
// ─────────────────────────────────────────────────────────────────────────────
...
function IconChevronFirst({ flip }: { flip?: boolean }) {
    return (
        <svg className={cn("w-3.5 h-3.5 shrink-0", flip && "rotate-180")} viewBox="0 0 16 16" {...ICON_PROPS}>
            <path d="M7 3L3 8l4 5M13 3L9 8l4 5" />
        </svg>
    );
}
function IconChevronLeft({ flip }: { flip?: boolean }) {
    return (
        <svg className={cn("w-3.5 h-3.5 shrink-0", flip && "rotate-180")} viewBox="0 0 16 16" {...ICON_PROPS}>
...
function IconChevronRight({ flip }: { flip?: boolean }) {
    return (
        <svg className={cn("w-3.5 h-3.5 shrink-0", flip && "rotate-180")} viewBox="0 0 16 16" {...ICON_PROPS}>
            <path d="M6 3l4 5-4 5" />
        </svg>
...
function IconChevronLast({ flip }: { flip?: boolean }) {
    return (
        <svg className={cn("w-3.5 h-3.5 shrink-0", flip && "rotate-180")} viewBox="0 0 16 16" {...ICON_PROPS}>
    ...
    return (
        <nav aria-label={t("navigation")} className="flex items-center flex-wrap gap-1">
            ...
            <NavButton
                ...
                className="w-9"
            >
                {/* In RTL: "first" points right, so the double-right icon */}
                <IconChevronFirst  />
                <IconChevronLast   />
            </NavButton>
...
// Default export alias so existing imports keep working
...
export function TablePagination({ pageCount, pageSize, total }: TablePaginationProps) {
    const t            = useTranslations("dashboard.pagination");
