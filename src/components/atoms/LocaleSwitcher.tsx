...
export default function LocaleSwitcher({ Trigger }: LocaleSwitcherProps) {
    ...
    return (
        <>
            {Trigger ? (
                <Trigger onClick={toggleLocale} disabled={isPending} lang={t('lang')} />
            ) : (
                <button
                    onClick={toggleLocale}
                    disabled={isPending}
                    // Replaced pure white with bg-dashboard-bg/off-white and added a soft ring
                    className="group cursor-pointer flex items-center gap-2 px-3 py-1.5 sm:px-4 sm:py-2 bg-slate-100/80 hover:bg-slate-200/80 border border-slate-200 transition-all rounded-xl active:scale-95 disabled:opacity-50"
                >
                    <div className="p-1 bg-white rounded-md shadow-sm">
                        <svg width="24" height="24" viewBox="0 0 28 28" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-5 h-5 transition-transform group-hover:rotate-12">
